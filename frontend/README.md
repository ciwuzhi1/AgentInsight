...
浏览器打开 **http://localhost:3100** 即可。需要后端（端口 8000）同时在跑；如果后端地址不同，可设置环境变量 `NEXT_PUBLIC_API_BASE` 指过去。

**常见坑**：页面报"网络错误"多半是后端没启动；改了代码没生效，刷新即可（dev 模式自动热更新，一般不需要手动重启）。

## 十、阶段2 新增：简历匹配与设置中心

页面区块顺序调整为：数据集上传① → 提问② → 时间线③ → 结果④ → **简历匹配⑤** → 爬虫面板⑥。时间线现在按后端 `plan` 事件渲染步骤卡片，依赖相同的步骤并排显示，重试/跳过也会实时标注；`final` 结果按 `engine` 字段自动区分数据形态与匹配形态（匹配分环形图 + 五维进度条 + 技能缺口 + LLM 解读）。

- **简历匹配⑤**：左侧上传 pdf/docx/txt 简历（≤10MB，重复上传会覆盖）；右侧勾选岗位（数据来自爬虫面板，最多 5 个，岗位库为空时先去⑥抓取）；点"开始匹配"后过程与结果复用上方的时间线和结果区。
- **提问② 为"岗位问题提示词"**：区块标题改为「提交岗位问题（提示词）」，原 4 个销售类示例按钮替换为 8 条岗位分析提示词预设（chips，针对 jd_large 数据集的 job_id/title/company/city/salary_k/skills/posted_date 字段设计）。点击 chip 只是把文本填进输入框，可编辑后再提交；也支持手输任意问题。预设与数据集无硬绑定——上传什么数据集由你决定，界面有小字提示"建议搭配 jd_large.csv 使用"。提交流程不变：`POST /api/tasks` 拿 task_id → SSE 实时渲染时间线。
- **设置中心在独立页面 /settings**：主页标题右侧有「⚙ 设置」链接跳转；设置页顶部有「← 返回分析台」链接。内容与原主页底部折叠面板一致（模型配置：新增 OpenAI 兼容模型、激活/测试连通/删除；开关与密钥：LLM 解读开关、兜底策略、PDF 解析后端、SQL 超时、MinerU/Tavily 密钥），逐项点"保存"生效，密钥留空表示不修改。区别是进入设置页即自动加载，不再需要展开折叠面板。
