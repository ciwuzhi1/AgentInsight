...
页面区块顺序调整为：数据集上传① → 提问② → 时间线③ → 结果④ → **简历匹配⑤** → 爬虫面板⑥ → **设置中心⑦**（原爬虫面板从⑤改为⑥）。时间线现在按后端 `plan` 事件渲染步骤卡片，依赖相同的步骤并排显示，重试/跳过也会实时标注；`final` 结果按 `engine` 字段自动区分数据形态与匹配形态（匹配分环形图 + 五维进度条 + 技能缺口 + LLM 解读）。

- **简历匹配⑤**：左侧上传 pdf/docx/txt 简历（≤10MB，重复上传会覆盖）；右侧勾选岗位（数据来自爬虫面板，最多 5 个，岗位库为空时先去⑥抓取）；点"开始匹配"后过程与结果复用上方的时间线和结果区。
- **设置中心⑦**：页面底部折叠面板，展开时自动加载。上半部分管理模型配置（新增 OpenAI 兼容模型、激活/测试连通/删除）；下半部分是开关与密钥（LLM 解读开关、兜底策略、PDF 解析后端、SQL 超时、MinerU/Tavily 密钥），逐项点"保存"生效，密钥留空表示不修改。
