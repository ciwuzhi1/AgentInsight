...
def test_match_plan_structure():
    """resume/job 无依赖，match 依赖二者，validator 依赖 match。"""
    steps = build_match_plan()
    by_id = {s.id: s for s in steps}
    assert set(by_id) == {"resume", "job", "match", "validator"}
...
def test_match_plan_topological_order():
    """validate_dag 返回拓扑序：resume/job 在 match 前，match 在 validator 前。"""
    ordered = validate_dag(build_match_plan())
    ids = [s.id for s in ordered]
    assert set(ids) == {"resume", "job", "match", "validator"}
...
def test_data_plan_structure():
    """data 无依赖，validator 依赖 data，且拓扑序 data → validator。"""
    ...
    assert [s.id for s in ordered] == ["data", "validator"]
