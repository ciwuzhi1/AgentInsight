"""DataAgent：画像 → 引擎路由 → NL2SQL → 组装 final_result（契约 §4/§6）。"""
...
# few-shot 历史是否已从磁盘加载（每个进程只加载一次）
_fewshot_history_loaded = False
    ...
    async def run(self, state: TaskState, emit: EmitFn) -> AgentResult:
        ...
        await emit(
            {
                ...
                "reason": (
                    ...
                    + (
                        f">= 阈值 {settings.MAX_ROWS_THRESHOLD}，数据量过大"
                        if engine == "too_large"
                        else f"低于阈值 {settings.MAX_ROWS_THRESHOLD}，走 DuckDB"
                    )
                ),
            }
        )
        ...
        if engine == "too_large":
            return AgentResult(
                ...
                data={},
                errors=[
                    f"数据集行数 {profile.rows_estimate} 超过阈值 "
                    f"{settings.MAX_ROWS_THRESHOLD}，暂不支持处理"
                ],
            )
        ...
        # 执行成功：写入 few-shot 历史库，并异步导出到磁盘（不阻塞）
